...
excerpt: "Five years inside a Fortune 100 enterprise where AI never got past autocomplete, then four months building an agent-driven practice that genuinely delivered — and the two problems left over that engineering couldn't solve on its own."
tag: "AI-Native Engineering"
date: 2026-07-28
readTime: "8 min read"
---

I spent most of fifteen years doing enterprise-shaped engineering, the last five of them inside a Fortune 100 enterprise. Then I spent four months at a startup where every piece of work moved through an agent. The four months rearranged more of my practice than the five years did.

Four months is short enough that you should be suspicious of anyone drawing conclusions from it, so let me argue that the compression is the finding rather than a caveat on it. Approaches that would have taken a year to evaluate under enterprise change control got evaluated in a week, which is the only reason a practice could converge that fast.
...
Because I don't think either extreme is a story about the amount of AI. The enterprise restricted the model heavily; the startup started out restricting it not at all. Both were adjusting the same dial. But how much you let a model generate isn't the variable that determines whether the output is safe. What determines that is how much structure sits on either side of the generation step — specification going in, verification coming out — and whether that structure was built for one person or for a team.

## Learning the tools by using them

I learned the tooling by running real work through it, not by reading about it. There's no substitute available; the useful knowledge is entirely in where things fail, and the failures aren't documented because they're specific to your repository, your conventions, and your team.
...
The fix isn't complicated, which is the encouraging part: an integration test audit, someone stepping back to write and tune tests against holistic business flows rather than individual tickets. The reason it doesn't happen on its own is the reason it's worth saying out loud — it isn't anybody's ticket. If you parallelize implementation across agents, verification of the seams stays stubbornly human for now, and it has to be made someone's explicit job.
...
The first was expectations. Leadership anchored on demos — theirs and the industry's — and the number that came out of watching a working slice appear in an hour became the number in the commitment.
...
We never fully closed that gap in four months. What it cost us was the ability to validate as thoroughly as we went, and it caused friction. I'd rather say that plainly than pretend the estimation conversation was solved.

The second was that people outside engineering could now build things that run. Prototypes started arriving as real, working applications — built by people who reasonably concluded that running meant nearly done. They came without the shared data layer, each owning its own database, and engineering inherited the reconciliation: pipelines to sync prototype data against what the production system actually ran on.
